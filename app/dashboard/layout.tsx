  ...
  return (
    <div className="flex min-h-screen bg-[#080808] text-white">
      <aside className="hidden md:flex fixed top-0 left-0 h-screen w-[72px] bg-[#0d0d0d] border-r border-white/5 flex-col items-center py-4 z-50">
        <Link href="/dashboard" id="tour-logo" className="mb-6 flex-shrink-0">
          <img src="https://lrdwqsllipznxajlyyea.supabase.co/storage/v1/object/public/media-uploads/1000055271-removebg-preview.png" alt="InfiMagen" className="w-8 h-8 rounded-full object-cover" />
        </Link>
        <nav className="flex-1 flex flex-col items-center gap-1 w-full px-2">
          {NAV.map((item, index) => {
            const active = item.href === "/dashboard" ? pathname === "/dashboard" : pathname.startsWith(item.href);
            const tourId = index === 0 ? "tour-nav-home" : index === 1 ? "tour-nav-library" : "tour-nav-bgremove";
            return (
              <Link key={item.label} href={item.href} id={tourId}
          ...
          <Link href="#" id="tour-nav-more" className="w-full flex flex-col items-center gap-1 py-2.5 px-1 rounded-xl text-white/30 hover:bg-white/5 hover:text-white/70 transition-all">
        ...
        <div className="flex flex-col items-center gap-2 w-full px-2 mb-2">
          <Link href="/dashboard/tts" id="tour-nav-settings" className="w-full flex flex-col items-center gap-1 py-2.5 px-1 rounded-xl text-white/30 hover:bg-white/5 hover:text-white/70 transition-all">
          ...
          <div id="tour-credits" className="flex items-center gap-1 bg-emerald-500/15 border border-emerald-500/25 rounded-lg px-2 py-1.5 w-full justify-center">
            <span className="text-emerald-400 text-xs">⬡</span>
            <span className="text-emerald-300 text-xs font-bold">{credits ?? "…"}</span>
          </div>
          <button id="tour-upgrade" onClick={() => router.push("/payment?plan=pro")}
            className="w-full bg-violet-600 hover:bg-violet-500 text-white text-[10px] font-bold py-1.5 rounded-lg transition-all">
            Upgrade
          </button>
          <button id="tour-profile" onClick={signOut} title="Sign out"
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 z-50 bg-[#0d0d0d]/95 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-4 h-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center">
        ...
        <div className="md:hidden fixed top-12 left-0 right-0 z-50 bg-[#111] border-b border-white/10 px-4 py-3 space-y-1">
          ...
          <div className="border-t border-white/5 pt-2 mt-2">
            <button onClick={() => router.push("/payment?plan=pro")}
              className="w-full bg-violet-600 text-white text-sm font-bold py-2.5 rounded-xl mb-2">Upgrade</button>
            ...
            <button onClick={signOut} className="w-full text-white/30 text-sm py-2 hover:text-white/60 transition-all">Sign Out</button>
          </div>
        </div>
      )}
